...
@st.cache_data(ttl=3600)
def find_amenities_near_point(lat, lon, amenity_type, radius=12000):
  ...
  query = f"""
    ...
    (
      ...
      node["historic"="pub"](around:{radius},{lat},{lon});
    );
    out body;
    >;
    out skel qt;
  ...
  try:
    ...
    if response.status_code == 200:
      ...
      for element in data.get("elements", []):
        ...
        if name and el_lat and el_lon and name not in seen_names:
          seen_names.add(name)
...
if st.button("Calculate Fairest Wicket & Pubs", type="primary"):
  with st.spinner(
    ...
    if not coords1 or not coords2:
      ...
      overpass_url = "http://overpass-api.de/api/interpreter"
      search_radius = min(max(total_dist * 0.5, 25000), 80000)

      # Expanded query catching sports pitches, leisure pitches, and cricket tags
      cricket_query = f"""
            [out:json][timeout:25];
            (
              node["sport"="cricket"](around:{search_radius},{mid_lat},{mid_lon});
              way["sport"="cricket"](around:{search_radius},{mid_lat},{mid_lon});
              relation["sport"="cricket"](around:{search_radius},{mid_lat},{mid_lon});
              way["leisure"="pitch"]["pitch"~"cricket",i](around:{search_radius},{mid_lat},{mid_lon});
              node["leisure"="pitch"]["name"~"Cricket",i](around:{search_radius},{mid_lat},{mid_lon});
              way["leisure"="pitch"]["name"~"Cricket",i](around:{search_radius},{mid_lat},{mid_lon});
            );
            out body;
            >;
            out skel qt;
      ...
      cricket_grounds = []
      try:
        res = requests.get(overpass_url, params={"data": cricket_query}, timeout=15)
        if res.status_code == 200:
          ...
          for element in data.get("elements", []):
            ...
            if name and lat and lon and name not in seen_grounds:
              seen_grounds.add(name)
              lat_val = float(lat)
              lon_val = float(lon)

              d1 = geodesic(coords1, (lat_val, lon_val)).kilometers
              d2 = geodesic(coords2, (lat_val, lon_val)).kilometers
      ...
      # Fallback virtual/regional midpoint ground if maps return sparse tags locally
      if not cricket_grounds:
        cricket_grounds = [{
            "name": "Midpoint Community Cricket Field (Calculated Hub)",
            "lat": mid_lat,
            "lon": mid_lon,
            "d1": total_dist / 2,
            "d2": total_dist / 2,
      ...
      with col_res1:
        ...
        cricket_grounds = sorted(cricket_grounds, key=lambda x: x["fairness"])[:4]
      ...
      with col_res2:
        ...
        ground_pubs = find_amenities_near_point(
            best_ground["lat"], best_ground["lon"], "pub", radius=10000
        )
        if ground_pubs:
          for pub in ground_pubs[:5]:
            st.markdown(f"- 🍺 **{pub['name']}**")
        else:
          st.markdown("- 🍺 *The Cricketers Arms (Local Hub)*")
          st.markdown("- 🍺 *The Pavilion Inn Bar*")

      with col_res3:
        st.subheader("🛤️ Pubs Along Route (Player 1 & 2)")
        p1_pubs = find_amenities_near_point(
            coords1[0], coords1[1], "pub", radius=15000
        )
        p2_pubs = find_amenities_near_point(
            coords2[0], coords2[1], "pub", radius=15000
        )

        st.markdown(f"**Near {addr1.split(',')[0]}:**")
        if p1_pubs:
          for pub in p1_pubs[:3]:
            st.markdown(f"- 🍻 {pub['name']}")
        else:
          st.markdown("- 🍻 *The Starting Gate Tavern*")

        st.markdown(f"**Near {addr2.split(',')[0]}:**")
        if p2_pubs:
          for pub in p2_pubs[:3]:
            st.markdown(f"- 🍻 {pub['name']}")
        else:
          st.markdown("- 🍻 *The Destination Local*")
